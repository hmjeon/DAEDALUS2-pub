...
It is note that DAEDALUS2 needs two files; geometry file and 'seq.txt' files
	2.1. Gemetry file
		The user can choose pre-defined target geometry when running DAEDALUS2. Also,
	...
	2.2. Scaffod sequence file
		...
		folder where DAEDALUS2.exe exists. If there is no file 'seq.txt', the m13mp18
...
3. Run on command with 9 input arguments:
		...
		DAEDALUS2 [output path] [geometry file] [sequence file] [edge-section] [vertex design] [edge] [edge length] [mesh spacing] [run mode]
		ex) DAEDALUS2   .\outputs   test.ply   seq.txt   3   1   42   0.0   s
	...
	para 2 - Goemetry file name including the path
		integer  - DAEDALUS2 uses the pre-defined geometry
		filename - DAEDALUS2 reads the user's geometry file
	...
	para 4 - Edge section (DAEDALUS2 always uses 1)
		1 - DX
		2 - 6HB (inner)
		3 - 6HB (middle)

	para 5 - Vertex design (DAEDALUS2 always uses 2)
		1 - Flat vertex
		2 - Mitered vertex
...
4. Examples of running DAEDALUS2 with input arguments

	For Windows
		DAEDALUS2.exe output 2D_1.ply seq.txt 1 2 0 42 0.0 s > DAEDALUS2.log 2>&1
	For Mac
		./DAEDALUS2 ./output 2D_1.ply seq.txt 1 2 0 63 0.0 s > DAEDALUS2.log 2>&1