...
subroutine Input_Initialize(prob, geom)
    ...
    if(iargc() == 0) then
        ...
        call Input_Print_Prob_DAEDALUS2
        read(*, *), c_prob
        ...
        if( prob.sel_edge_sec /= 1 ) then
            write(p_redir, "(a)")
            write(p_redir, "(a)"), " +=== error ========================================+"
            write(p_redir, "(a)"), " | Please use the proper edge section.              |"
            write(p_redir, "(a)"), " +==================================================+"
            stop
        end if
    ...
    prob.sel_vertex = 2
    para_dist_pp    = 0.6d0      ! [0.42, 0.6], distance b/w two nts, nm
    if(prob.sel_edge_sec /= 1) stop
...
subroutine Input_Set_Init(prob)
    ...
    if(p_redir /= 0) then
        open(unit = p_redir, file = "DAEDALUS2.log", form = "formatted")
    end if
...
! Print pre-defined geometries for DAEDALUS2
subroutine Input_Print_Prob_DAEDALUS2
    write(0, "(a)")
    write(0, "(a)"), "   +===================================================================+"
    write(0, "(a)"), "   |                                                                   |"
    write(0, "(a)"), "   |        DAEDALUS2 for the 3D DNA wireframe with DX edges           |"
    ...
    write(0, "(a)"), "       1. Tetra,    2. Cube,    3. Octa,    4. Dodeca,    5. Icosa"
    write(0, "(a)")
    write(0, "(a)"), "    [ Irregular geometries #1; arbitrary vertex but equal edge length ]"
    write(0, "(a)"), "       6. Penta Pyramid,      7. Penta Cupola,       8. Hexa Prism"
    write(0, "(a)"), "       9. Penta Antiprism,   10. Chiral Object"
    write(0, "(a)")
    write(0, "(a)"), "    [ Irregular geometries #2; arbitrary vertex and edge length ]"
    write(0, "(a)"), "      11. Asym tetra,        12. Twisted Cube,      13. Twisted Tri Prism"
    write(0, "(a)"), "      14. Bi Propello Tetra, 15. Bi Propello Cube"
    write(0, "(a)")
    write(0, "(a)"), "    [ Irregular geometries #3; End-open highly irregular geometries ]"
    write(0, "(a)"), "      16. Cylinder (TRI),    17. Hemisphere (QUAD), 18. Hemisphere (TRI)"
    write(0, "(a)"), "      19. Hourglass (QUAD),  20. Hourglass (TRI)"
    write(0, "(a)")
    write(0, "(a)"), "      21. Penta Bipyramid,   22. Cubeoctahedron"
    write(0, "(a)")
    write(0, "(a)"), "   Select the number or type geometry file (*.ply) [Enter] : "
    write(0, "(a)")
end subroutine Input_Print_Prob_DAEDALUS2
...
subroutine Input_Set_Prob(prob, geom)
    ...
    if(prob.sel_prob /= 0) then
        call Input_Sel_Prob_DAEDALUS2(prob, geom)
    end if
...
! Select the pre-defined geometry for DAEDALUS2
subroutine Input_Sel_Prob_DAEDALUS2(prob, geom)
    type(ProbType), intent(inout) :: prob
    type(GeomType), intent(inout) :: geom
    ...
    select case (prob.sel_prob)
        ...
        ! Platonic solids
        case ( 1); call Exam_DAEDALUS2_Tetrahedron  (prob, geom)
        case ( 2); call Exam_DAEDALUS2_Cube         (prob, geom)
        case ( 3); call Exam_DAEDALUS2_Octahedron   (prob, geom)
        case ( 4); call Exam_DAEDALUS2_Dodecahedron (prob, geom)
        case ( 5); call Exam_DAEDALUS2_Icosahedron  (prob, geom)

        ! Irregular; Arbitrary vertex angle but equal edge length
        case ( 6); call Exam_DAEDALUS2_Penta_Pyramid     (prob, geom)
        case ( 7); call Exam_DAEDALUS2_Penta_Cupola      (prob, geom)
        case ( 8); call Exam_DAEDALUS2_Prism_Hexa        (prob, geom)
        case ( 9); call Exam_DAEDALUS2_Anti_Prism_Penta  (prob, geom)
        case (10); call Exam_DAEDALUS2_Chiral_Object     (prob, geom)

        ! Irregular; Arbitrary vertex angle and edge length
        case (11); call Exam_DAEDALUS2_Asym_Tetra        (prob, geom)
        case (12); call Exam_DAEDALUS2_Twisted_Cube      (prob, geom)
        case (13); call Exam_DAEDALUS2_Twisted_Tri_Prism (prob, geom)
        case (14); call Exam_DAEDALUS2_Bi_Propello_Tetra (prob, geom)
        case (15); call Exam_DAEDALUS2_Bi_Propello_Cube  (prob, geom)

        ! End-open highly irregular geometries
        case (16); call Exam_DAEDALUS2_End_Cylinder_Tri  (prob, geom)
        case (17); call Exam_DAEDALUS2_Hemisphere_Quad   (prob, geom)
        case (18); call Exam_DAEDALUS2_Hemisphere_tri    (prob, geom)
        case (19); call Exam_DAEDALUS2_Hourglass_Quad    (prob, geom)
        case (20); call Exam_DAEDALUS2_Hourglass_Tri     (prob, geom)

        case (21); call Exam_DAEDALUS2_Penta_Bipyramid (prob, geom)
        case (22); call Exam_DAEDALUS2_Cubeocta        (prob, geom)
...
end subroutine Input_Sel_Prob_DAEDALUS2
